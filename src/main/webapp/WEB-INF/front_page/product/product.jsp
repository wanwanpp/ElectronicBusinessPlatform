            ...
            <c:if test="${!empty brands}">
                <li><b>品牌：</b>
                    <p>
                        ...
                        <a href="/product/list/display.shtml?brandId=1&brandName=依琦莲&typeName=${typeName }&typeId=${typeId}"
                           title="依琦莲">依琦莲</a>
                        <c:forEach items="${brands}" var="brand">
                            <a href="javascript:void(0)" title="${brand.name}"
                               onclick="window.location.href='/product/display/list.shtml?' +
                                       'brandId=${brand.id}&brandName=${brand.name}&typeId=${typeId}&typeName=${typeName}'">${brand.name}</a>
                        </c:forEach>
                    </p></li>
            </c:if>
            <li><b>价格：</b>
        ...
        <div class="mt ofc">
            <ul class="uls i_150x150 x4_150x150b">
                ...
                <c:forEach items="${pagination.list}" var="entry">
                    <li>
                        <a href="javascript:void(0)" title="${entry.name}" class="pic"
                           onclick="window.open('/html/product/${entry.id}.html')">
                            <img src="${entry.img.allUrl}" alt="${entry.name}"/></a>
                        <dl>
                            <!-- dt 10个文字+... -->
